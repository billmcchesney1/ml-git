...
class CloneTest(unittest.TestCase):
    ...
    def test_01_clone(self):
        self.set_up_clone()
        self.assertIn(output_messages['INFO_SUCCESSFULLY_LOAD_FILES'],
    ...
    def test_03_clone_wrong_url(self):
        self.assertIn(output_messages['ERROR_COULD_NOT_READ_FROM_REPOSITORY'],
                      check_output(MLGIT_CLONE % (self.GIT_CLONE + "wrong", "--folder=" + CLONE_FOLDER)))
    ...
    def test_04_clone_inside_folder_without_permission(self):
        self.set_up_clone()
        self.assertIn(output_messages['ERROR_PERMISSION_DENIED_IN_FOLDER'] % os.path.join(PATH_TEST, 'test_permission'),
                      check_output(MLGIT_CLONE % (self.GIT_CLONE, "--folder=" + CLONE_FOLDER)))
    ...
    def test_05_clone_folder_with_track(self):
        self.set_up_clone()
        self.assertIn(output_messages['INFO_SUCCESSFULLY_LOAD_FILES'],
                      check_output((MLGIT_CLONE + ' --track') % (self.GIT_CLONE,
                                                                 "--folder=" + CLONE_FOLDER)))
    ...
    def test_06_clone_without_folder_option(self):
        ...
        os.mkdir(CLONE_FOLDER)
        os.chdir(CLONE_FOLDER)
        self.assertIn(output_messages['INFO_SUCCESSFULLY_LOAD_FILES'], check_output(MLGIT_CLONE % (self.GIT_CLONE, "")))

    @pytest.mark.usefixtures("start_local_git_server")
    def test_07_clone_to_folder_without_permission(self):
        self.set_up_clone()
        os.chdir(PATH_TEST)
        self.assertIn(output_messages['ERROR_PERMISSION_DENIED_IN_FOLDER'] % '',
                      check_output(MLGIT_CLONE % (self.GIT_CLONE, "--folder=test_permission")))
    ...
    def test_08_clone_repository_with_wrong_configurations(self):
        ...
        self.assertIn(output_messages['ERROR_COULD_NOT_INITIALIZE_METADATA'] % 'dataset',
                      check_output(MLGIT_CLONE % (self.GIT_CLONE, '--folder=' + CLONE_FOLDER)))
